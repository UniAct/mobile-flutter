...
@TableIndex(name: 'idx_sync_queues_device_priority', columns: {#deviceId, #priority})
@TableIndex(name: 'idx_sync_queues_next_retry', columns: {#nextRetryAt})
class SyncQueues extends Table {
  IntColumn get id => integer().autoIncrement()();

  TextColumn get actionType => text()(); // 'create', 'update', 'delete', 'sync_attendance'
  TextColumn get entityType => text()(); // 'attendance', 'session', 'student', 'qr_mapping'
  ...
  TextColumn get syncStatus => text().withDefault(const Constant('pending'))(); // 'pending', 'syncing', 'success', 'failed', 'cancelled'
